...
class View(tk.Tk):
    ...
    def inv_button_click_1(self):
        myInv = Inventory()
        myInv.addItem("chicken", 45)
        myInv.__str__()

    def inv_button_click_2(self):
        myInv = Inventory()
        myInv.addItem("carrots", 15)
        myInv.__str__()

    def inv_button_click_3(self):
        myInv = Inventory()
...
class Order(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button_1 = tk.Button(self, bg='#FFA500',  fg='#FFFFFF', text="Create Order", padx=20, pady=10, font=controller.button_font,
                             command=lambda: controller.show_frame("Create_order"))
                            #getOrder, addOrder,  function from Nathan's code
        button_2 = tk.Button(self,  bg='#FFA500',  fg='#FFFFFF', text="Edit Order", padx=20, pady=10, font=controller.button_font,
                             command=lambda: self.controller.edit_order_click(self))
                            #getAllActiveOrders, #removeOrder from Nathan's Code
...
class Checkout(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button_1 = tk.Button(self, bg='#FFA500', fg='#FFFFFF', text="Total", padx=20, pady=10,
                            ...
                            #orderOut from Nathan's Code in Order.py will show orderId, customername, items and total
...
class Create_order(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        button_1 = tk.Button(self, bg='#FFA500', fg='#FFFFFF', text="Add", padx=20, pady=10, font=controller.button_font,
                             command=lambda: controller.show_frame("New"))
                    #
        button_2 = tk.Button(self, bg='#FFA500', fg='#FFFFFF', text="Delete", padx=20, pady=10, font=controller.button_font,
                             command=lambda: controller.show_frame("Create_order"))
        button_3 = tk.Button(self, text="Create", padx=20, pady=10, font=controller.button_font,
                             command=lambda: controller.show_frame("Create_order"))
        button_4 = tk.Button(self, text="Go back to Main Menu", padx=20, pady=10, font=controller.button_font,
                             command=lambda: controller.show_frame("StartPage"))
        button_1.pack(fill=tk.X)
        button_2.pack(fill=tk.X)
        button_3.pack(fill=tk.X)
        button_4.pack(fill=tk.X)
...
class New(tk.Frame):
    ...
    def TextBox(self):
        ...
        button = tk.Button(self, text="View Order", bg='#FFA500', fg='#FFFFFF', padx=20, pady=10,
                           font=button_font_1, command=self.text)
        button.grid(column=0, row=1, columnspan=2)
        text_box = tk.Text(self, wrap='word', font=text_font_1, height=30, width=50)
        text_box.grid(column=0, row=0, columnspan=2, sticky='NSWE', padx=5, pady=3)
        sys.stdout = StdoutRedirector(text_box)
...
class Expand_Order(tk.Frame):
    def __init__(self, parent,controller):
        ...
        button_1 = tk.button(self, text="Create", padx=20, pady=20, font=controller.buttin_font,
